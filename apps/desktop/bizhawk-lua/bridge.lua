...
-- Reads the CURRENT real input state (physical controller/keyboard, not
-- anything this bridge itself set) — used for recording a human playing,
-- as opposed to press_buttons/play_input_sequence which SEND input.
local function cmd_get_buttons(p)
    if not CAPS.joypad_get then error("joypad.get not available") end
    local player = p.player or 1
    return joypad.get(player)
end

-- Cheap combined read for recording a human's play in real time: current
-- framecount (so the Node side can detect how many real frames elapsed
-- between polls, since polling is slower than 60fps and must NOT itself
-- drive frame advancement — the outer loop's own emu.frameadvance() already
-- does that at whatever speed BizHawk is actually running) plus the
-- current real button state, in one round trip instead of two.
local function cmd_get_input_snapshot(p)
    local player = p.player or 1
    return {
        framecount = CAPS.framecount and emu.framecount() or nil,
        buttons = CAPS.joypad_get and joypad.get(player) or {},
    }
end
...
local HANDLERS = {
    ...
    get_buttons          = cmd_get_buttons,
    get_input_snapshot   = cmd_get_input_snapshot,
...
while true do
    ...
    if tick_count % 60 == 0 then
        ...
        local f = io.open(temp_dir .. "\\regs_out.txt", "w")
        if f then
            f:write("Probing registers:\n")
            local reg_names = {"PC", "A", "X", "Y", "S", "P", "D", "DB", "E", "PBR", "DBR", "Status"}
            for _, name in ipairs(reg_names) do
                local ok, val = pcall(emu.getregister, name)
                if ok then
